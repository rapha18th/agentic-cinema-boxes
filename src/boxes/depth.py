...
# Audio and video budgets run high on purpose: the multimodal index is the
# thing to show, so a run reaches for recordings whenever an open one exists.
SCOUT = Depth("scout", objectives=5, queries_per_objective=1, extract_urls=2,
              full_content=False, images_per_round=4, docs_per_round=2, av_per_round=4,
...
PRODUCTION = Depth("production", objectives=10, queries_per_objective=2, extract_urls=4,
                   full_content=True, images_per_round=10, docs_per_round=5, av_per_round=8,
...
KUBRICK = Depth("kubrick", objectives=16, queries_per_objective=3, extract_urls=6,
                full_content=True, images_per_round=20, docs_per_round=10, av_per_round=14,
